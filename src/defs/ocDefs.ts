...
tmpClassDef = new LuaClass("event");
tmpClassDef.description = "The Event API provides a basic event system to allow your code to react to signals sent by the OS or other programs/libraries.\nFor example, this can be used to capture keys pressed, react if an external screen is attached or removed, or handle incoming network messages.";
...
tmpClassDef.methods.push(tmpDef);
...
OpenComputerClassDefinitions.push(tmpClassDef);
